...
Styler.SpatialFilterPanel = Ext.extend(Styler.BaseFilterPanel, {
    ...
    createBuffer: function(size) {
        ...
        // The given buffer size is "ground size"...
        // which might not be the same in the projection plane.
        // Get the correct buffer size.
        var cll = this.feature.geometry.getBounds().getCenterLonLat(),
        cg = new OpenLayers.Geometry.Point(cll.lon, cll.lat).transform(
            this.map.projection, "EPSG:4326"),
        ll2 = OpenLayers.Util.destinationVincenty({
            lon: cg.x,
            lat: cg.y
        }, 0, size),
        g2 = new OpenLayers.Geometry.Point(ll2.lon, ll2.lat).transform(
            "EPSG:4326", this.map.projection),
        newsize = new OpenLayers.Geometry.Point(
            cll.lon, cll.lat).distanceTo(g2);

        OpenLayers.Request.POST({
            url: this.bufferService + Math.round(newsize*1000)/1000,
