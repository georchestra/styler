                        ...
                        success: function(form, action) {
                            ...
                            OpenLayers.Request.GET({
                                ...
                                success: function(response) {
                                    ...
                                    //Root of cloned filters is a logical filter,
     ...
     _loadfilters: function (filter) {
        if (filter instanceof OpenLayers.Filter.Logical) {
            ...
            if (! (stylerType[filter.type] === undefined)) {
                this.findByType("gx_filterbuilder")[this.findByType("gx_filterbuilder").length-1].findByType("combo")[0].setValue(comboType[filter.type]);
                this.findByType("gx_filterbuilder")[this.findByType("gx_filterbuilder").length-1].changeBuilderType(stylerType[filter.type]);
            }

            var gxFilterBuilderChildren = this.childFiltersPanel.findByType("gx_filterbuilder");
            Ext.each(filter.filters, this._loadfilters, gxFilterBuilderChildren[gxFilterBuilderChildren.length - 1]);
        ...
        if (filter instanceof OpenLayers.Filter.Spatial) {
            ...
            this.childFiltersPanel.remove(this.childFiltersPanel.items.items[this.childFiltersPanel.items.getCount()-1].getId());
