...
Styler.FilterBuilder = Ext.extend(Ext.Panel, {
    ...
    /**
     * Private property, display debug message
     */
    _debug: false,
                    ...
                    form.submit({
                        ...
                        success: function(form, action) {
                            ...
                            OpenLayers.Request.GET({
                                ...
                                success: function(response) {
                                    var filterString, fx, ff, loadedFilters, clonedFilters, xmlFilters;
                                    filterString = response.responseText;
                                    ...
                                    if (clonedFilters instanceof OpenLayers.Filter.Logical) {
                                        ...
                                        if (!(stylerType[clonedFilters.type] === undefined)) {
                                            this._dbg("Root case, Combo and Builder Type set to " + clonedFilters.type);
                                        ...
                                        if ((clonedFilters.type == "&&") || (clonedFilters.type == "||")) {
                                            this._dbg("Root case, calling _loadfilter on a group of type " + clonedFilters.type);

                                            Ext.each(clonedFilters.filters, this._loadfilters, this);
                                        } else {
                                            //NOT : we already change the combobox, now loaded child filters
                                            this._dbg("Root case, calling _loadfilter on a condition inside the root not");
                                        ...
                                        this._dbg("Root case, calling _loadfilter on a condition inside the root");
    ...
    _loadfilters: function(filter) {
        if (filter instanceof OpenLayers.Filter.Logical) {
            ...
            if (!(stylerType[filter.type] === undefined)) {
                this._dbg("generic case, Combo and Builder Type set to" + filter.type);
            ...
            Ext.each(filter.filters, function(filter) {
                this._dbg("generic case, call _loadfilter on filter of type " + filter.type);
        ...
        if (filter instanceof OpenLayers.Filter.Comparison) {
            this._dbg("generic case, adding a comparison condition");
        ...
        if (filter instanceof OpenLayers.Filter.Spatial) {
            ...
            this._dbg("generic case, adding a spatial condition");
